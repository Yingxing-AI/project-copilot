...
## 2026-06-17 OSS Wording Softening Pass

- 日期：2026-06-17
- 完成内容：继续软化 OSS 次级工作流文案，调整开源准备、云端备份和公开阅读文档中的工程术语暴露，并补充对应测试断言。
- 遇到问题：部分辅助文档仍保留仓库平台术语，容易让“软化”只停留在部分输出层。
- 明日计划：继续检查剩余的 OSS 辅助文案，确认用户可见内容已经统一为更中性的表达。

## 2026-06-17 Softening Scope Correction

- 日期：2026-06-17
- 完成内容：收回过度软化的部分，恢复文档里的必要 GitHub 语境，并把测试从全局否定改为只约束用户交互输出。
- 遇到问题：过宽的“不要出现 GitHub”式约束会误伤技术精确性，和“合理即可”的目标不一致。
- 明日计划：继续沿着“只软化用户交互、不抹掉必要技术精度”的边界维护文案。

## 2026-06-17 dashboard Case Study Entry

- 日期：2026-06-17
- 完成内容：为 `dashboard` 新增 case study 入口，并把它登记进验证报告，当前状态标为待接入。
- 遇到问题：该项目暂时没有接入的 `.ai/` 运行记录，且后续只能在用户明确提供项目目录后做非侵入式读取。
- 明日计划：如果接入该项目根目录，再补齐真实工作日志、决策、知识沉淀和运行观察。
