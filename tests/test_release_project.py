...
class ReleaseProjectTest(unittest.TestCase):
    ...
    def test_release_runs_push_tag_and_gh_release_in_order(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            outcome = release_project(root, "v0.3.0-alpha.3", runner=runner)

            self.assertEqual(outcome.status, "success")
            self.assertIn(["git", "push", "origin", "main"], calls)
            self.assertIn(["git", "tag", "-a", "v0.3.0-alpha.3", "-m", "Project Copilot v0.3.0-alpha.3"], calls)
            self.assertIn(["git", "push", "origin", "v0.3.0-alpha.3"], calls)
    ...
    def test_release_dry_run_does_not_push_or_tag(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            outcome = release_project(root, "v0.3.0-alpha.3", runner=runner, dry_run=True)
