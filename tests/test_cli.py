...
class CliTest(unittest.TestCase):
    ...
    def test_interactive_mode_shows_summary_runs_until_exit(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("已生成 PROJECT_CHARTER.md、PROJECT_CONTEXT.md、STATUS.md、ROADMAP.md、DECISIONS.md、adr/ 和 sessions/", rendered)
    ...
    def test_startup_secretary_intro(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(exit_code, 0)
            self.assertIn("你好，我是你的项目秘书。", rendered)
            self.assertIn("* 记住项目背景", rendered)
            self.assertIn("* 保存关键决策原因", rendered)
